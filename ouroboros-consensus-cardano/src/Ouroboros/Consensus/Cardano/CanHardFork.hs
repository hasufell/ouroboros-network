...
instance ShelleyConstraints era => SingleEraBlock (ShelleyBlock era) where
  singleEraTransition pcfg _eraParams _eraStart ledgerState =
...
instance ShelleyConstraints era => HasPartialLedgerConfig (ShelleyBlock era) where
...
type CardanoHardForkConstraints c =
  ( TPraosCrypto c
  , ShelleyConstraints (ShelleyEra c)
  , ShelleyConstraints (AllegraEra c)
  , ShelleyConstraints (MaryEra    c)
...
translateHeaderHashByronToShelley ::
     forall c. (Crypto c, HASH c ~ Blake2b_256)
...
instance ( Era era
  ...
  type TranslationError era (GenTx :.: ShelleyBlock) = ()
...
translateChainDepStateShelleyToAllegraWrapper =
    ignoringBoth $
      Translate $ \_epochNo (WrapChainDepState chainDepState) ->
        -- Same protocol, same 'ChainDepState'
