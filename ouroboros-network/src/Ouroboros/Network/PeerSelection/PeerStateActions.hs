...
-- is passed from 'ConnectionHandler' via 'MuxPromise'.  This variable allows
  ...
  where
    ...
    establishPeerConnection jobPool remotePeerAddr =
      ...
      bracketOnError
        ...
        $ \peerStateVar -> do
          (muxHandle :: MuxHandle muxMode peerAddr versionNumber ByteString m a b)
          ...
          case muxHandle of
            MuxRunning connectionId@ConnectionId { localAddress, remoteAddress }
                       ...
                       mux
                       muxBundle
                       controlMessageVarBundle -> do

              atomically $ do
                writeTVar (projectBundle TokHot         controlMessageVarBundle) Terminate
                writeTVar (projectBundle TokWarm        controlMessageVarBundle) Continue
                writeTVar (projectBundle TokEstablished controlMessageVarBundle) Continue
            ...
            MuxPromiseHandshakeClientError err -> do
              ...
              throwIO (EstablishConnectionClientHandshakeException err)

            MuxPromiseHandshakeServerError err -> do
              traceWith spsTracer (PeerStatusChangeFailure
                                    (ColdToWarm remotePeerAddr Nothing)
                                    HandshakeServerFailure)
              throwIO (EstablishConnectionServerHandshakeException err)
      ...
      case res of
        Nothing -> do
          ...
          traceWith spsTracer (PeerStatusChangeFailure
                                ...
                                TimeoutError)
          throwIO (PeerDeactivationTimeoutException pchConnectionId)
    ...
    closePeerConnection
      ...
      case res of
        Nothing -> do
          ...
          throwIO (PeerCloseConnectionTimeoutException pchConnectionId)
...
mkApplicationHandleBundle
    ...
    -> Bundle (ApplicationHandle muxMode bytes m a b)
mkApplicationHandleBundle muxBundle controlMessageVarBundle awaitVarsBundle =
    Bundle
    ...
    mkApplication tok =
      let app =
            ApplicationHandle {
              ...
              ahControlVar          = projectBundle tok controlMessageVarBundle,
