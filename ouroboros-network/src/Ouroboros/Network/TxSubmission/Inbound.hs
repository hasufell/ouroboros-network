...
data TraceTxSubmissionInbound txid tx
    ...
    = TxInboundTerminated
    | TxInboundCanRequestMoreTxs Int
    | TxInboundCannotRequestMoreTxs Int
  deriving Show
    ...
    serverIdle n = StatefulM $ \st -> case n of
        Zero -> do
          ...
          then do
            ...
            traceWith tracer (TxInboundCanRequestMoreTxs (natToInt n))
            pure $ continueWithState (serverReqTxs Zero) st

          else do
            traceWith tracer (TxInboundCannotRequestMoreTxs (natToInt n))
            ...
            assert (requestedTxIdsInFlight st == 0
              ...
              SendMsgRequestTxIdsBlocking
                ...
                (traceWith tracer TxInboundTerminated)
